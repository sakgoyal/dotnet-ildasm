...
namespace DotNet.Ildasm.Tests.Infrastructure
{
    public class PropertyDefinitionExtensionsShould
    {
        ...
        private static readonly string[] WPM = [
        ...
        private static readonly string[] WPSM = [
        ...
        public void Write_Custom_Attributes()
        {
            ...
            var propertyDefinition = type.Properties.First(static x => x.Name == "SomePropertyWithAttribute");
        ...
        {
            var type = DataHelper.SampleAssembly.Value.Modules.First().Types.First(static x => x.Name == "SomeClassWithAttribute");
            var propertyDefinition = type.Properties.First(static x => x.Name == "SomeStaticPropertyWithAttribute");

            propertyDefinition.WriteILBody(_outputWriterMock);
            _outputWriterMock.Received(2).WriteLine(Arg.Is<string>(
                static x => WPSM.Contains(x)
            ));
        }
    }
}
